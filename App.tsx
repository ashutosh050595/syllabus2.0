...
import { ClipboardList, Users, LogIn, ShieldCheck, Zap, User, Loader2, FileText, Printer, MessageCircle, Mail } from 'lucide-react';
...
const App: React.FC = () => {
        ...
        if (user) {
          ...
          if (normalizedUserEmail === adminEmail) {
            ...
            if (teacher) {
              ...
              // Final fallback check with fresh fetch to avoid any cache issues
              ...
              if (freshTeacher) {
                ...
                alert(`System Alert: Your profile (${user.email}) was not found in the School Registry. Please ask Admin to seed the registry or add you manually.`);
  ...
  const shareViaWhatsApp = () => {
    const text = `Sacred Heart Koderma - Weekly Syllabus for Class ${selectedClass} is ready. Please check the portal.`;
    window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank');
  };

  const shareViaEmail = () => {
    const subject = `Weekly Syllabus Digest - Class ${selectedClass}`;
    const body = `The weekly syllabus for Class ${selectedClass} has been compiled and is ready for review.`;
  ...
  if (isAuthenticating && !state.currentUser) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6">
        ...
        <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Verifying Institutional Registry...</p>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center p-6">
        <div className="max-w-md w-full glass-card p-10 rounded-3xl shadow-2xl animate-in zoom-in-95">
          <div className="text-center mb-10">
            ...
            <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-2">Faculty Management Cloud</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <input type="email" required placeholder="name@sacredheartkoderma.org" className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl font-bold focus:border-indigo-600 outline-none" value={loginForm.email} onChange={e => setLoginForm({...loginForm, email: e.target.value})} />
        ...
        <div className="space-y-10">
          <div className="flex flex-wrap justify-center gap-2">
            <div className="bg-white p-1.5 rounded-2xl border border-slate-200 shadow-sm flex gap-1">
              ...
              <button onClick={() => setActiveTab('compile')} className={`flex items-center gap-2 px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'compile' ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400 hover:text-indigo-600'}`}><FileText className="h-4 w-4" /> Compilation Tool</button>
            </div>
          </div>
            ...
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col xl:flex-row justify-between items-center gap-6">
                <div className="text-center xl:text-left">
                  <h3 className="text-2xl font-black text-slate-900 italic tracking-tight">Academic Syllabus Compiler</h3>
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1">Class-wise view with submission gap detection</p>
                </div>
                <div className="flex flex-wrap justify-center gap-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => window.print()} className="bg-slate-900 text-white px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 hover:bg-black transition-all shadow-md"><Printer className="h-4 w-4" /> Print PDF</button>
          ...
          {activeTab === 'plans' && (
            <div className="space-y-8 animate-in fade-in">
              <div className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-3xl border border-slate-200 shadow-sm gap-6">
                <div>
                  <h3 className="text-2xl font-black text-slate-900 italic tracking-tight flex items-center gap-3"><Zap className="h-6 w-6 text-indigo-600" /> Academic Auditor</h3>
                  <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">Evaluate curriculum progress and pedagogical consistency</p>
                </div>
                <button onClick={async () => { setIsAuditing(true); const res = await APIService.generateAIAudit(state.lessonPlans); setAuditResult(res); setIsAuditing(false); }} disabled={isAuditing || state.lessonPlans.length === 0} className="bg-emerald-600 text-white px-10 py-4 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-50">{isAuditing ? 'Analyzing...' : 'Run Global Audit'}</button>
              </div>
