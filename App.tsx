...
const App: React.FC = () => {
  ...
  // Default to Registry so admin sees data immediately
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400">Sacred Heart Infrastructure</p>
        </div>
      </div>
    );
  }
      ...
      <div className="min-h-screen flex items-center justify-center p-6 bg-[#f8fafc]">
        <div className="max-w-md w-full">
          ...
          <div className="bg-white p-2 rounded-[2rem] border border-slate-200 shadow-xl flex gap-1 mb-6">
            <button onClick={() => setLoginMode('teacher')} className={`flex-1 py-3.5 rounded-2xl font-black text-[9px] uppercase tracking-widest transition-all ${loginMode === 'teacher' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'text-slate-400'}`}>Teacher Login</button>
          ...
          <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-2xl">
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <button type="submit" disabled={isSyncing} className={`w-full font-black py-4 rounded-2xl shadow-xl uppercase tracking-widest text-xs transition-all ${loginMode === 'admin' ? 'bg-slate-900 text-white' : 'bg-indigo-600 text-white'}`}>
                {isSyncing ? <RefreshCw className="h-4 w-4 animate-spin mx-auto" /> : 'Sign In'}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="space-y-8">
          ...
          {['plans', 'compile', 'history', 'logins'].includes(activeTab) && (
            <div className="bg-white p-20 rounded-[3rem] border border-slate-200 shadow-sm text-center">
              ...
              <h3 className="text-xl font-black uppercase italic tracking-tight mb-2">Detailed Module: {activeTab.toUpperCase()}</h3>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] max-w-sm mx-auto">This section is currently indexing institutional data for real-time reporting.</p>
            </div>
          )}
        </div>
      ) : (
